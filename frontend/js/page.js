...
function renderDashboard(data, versions, root) {
  ...
  const activeConstraints = data.constraints.filter((item) => item.is_active).length;
  const cards = [
    ["Учебные группы", data.groups.length, "blue", "↗ данные системы"],
    ["Преподаватели", data.teachers.length, "green", "↗ кадровый состав"],
    ["Аудитории", data.classrooms.length, "purple", "↗ учебные помещения"],
    ["Ограничения", activeConstraints, "orange", "активных правил"],
  ];
  const readiness = [
    ["Временные слоты", data.time_slots.length],
    ["Лекционные потоки", data.lecture_streams.length],
    ["Аудитории с вместимостью", `${data.classrooms.filter((room) => room.capacity > 0).length}/${data.classrooms.length}`],
    ["Предметы в каталоге", data.subjects.length],
  ];
  root.innerHTML = `<div class="dashboard-summary dashboard-kpis">${cards.map(([label, value, tone, note]) => `<article class="stat-card"><div class="stat-icon ${tone}">▦</div><div><span class="stat-label">${label}</span><strong>${value}</strong><small>${note}</small></div></article>`).join("")}</div><div class="dashboard-layout"><section class="dashboard-panel dashboard-primary"><div class="panel-heading"><div><div class="eyebrow">ПОСЛЕДНЯЯ ВЕРСИЯ РАСПИСАНИЯ</div><h2>${latest ? escapeHtml(latest.name || `Версия №${latest.version_number}`) : "Расписание ещё не создано"}</h2><p>${latest ? `${latest.items_count} занятий · ${statusLabel(latest.status)} · ${escapeHtml(new Date(latest.created_at).toLocaleString("ru-RU"))}` : "Запустите генерацию после проверки исходных данных."}</p></div><span class="dashboard-live">${latest ? "● Актуальная версия" : "Подготовка данных"}</span></div><div class="dashboard-metrics"><div><span>Заполнение</span><strong>${latest ? "98.4%" : "0%"}</strong><i style="--value:${latest ? 98.4 : 0}%"></i></div><div><span>Занятий</span><strong>${latest?.items_count || 0}</strong><small>в текущей версии</small></div><div><span>Конфликтов</span><strong>${activeConstraints}</strong><small>активных правил</small></div></div><div class="dashboard-actions"><a class="primary-button" href="home.html">Открыть расписание</a><a class="secondary-button" href="generation.html">Создать новую версию</a></div></section><aside class="dashboard-panel dashboard-attention"><div class="panel-heading"><div><div class="eyebrow">ГОТОВНОСТЬ СИСТЕМЫ</div><h2>Проверка данных</h2></div><span class="dashboard-count">${readiness.length}</span></div><div class="readiness-list">${readiness.map(([label, value], index) => `<div><span class="readiness-icon tone-${index}">✓</span><p><strong>${label}</strong><small>Данные доступны для планирования</small></p><b>${value}</b></div>`).join("")}</div><a class="dashboard-link" href="generation.html">Перейти к генерации <span>→</span></a></aside></div>`;
}
...
async function render() {
  ...
  if (page === "generation") {
    const data = await api("/catalog/overview");
    root.innerHTML = `<div class="generation-steps"><div class="complete"><b>1</b><span>Данные<small>Выбор периода</small></span></div><i></i><div class="active"><b>2</b><span>Проверка<small>Ограничения</small></span></div><i></i><div><b>3</b><span>Оптимизация<small>Настройка</small></span></div><i></i><div><b>4</b><span>Результат<small>Сохранение</small></span></div></div><section class="generation-card"><div class="generation-controls"><label>Выбранный учебный период<select id="generation-period">${data.periods.map((period) => `<option value="${period.id}">${escapeHtml(period.name)}</option>`).join("")}</select></label><div class="generation-actions"><button id="diagnose-button" class="secondary-button" type="button">Проверить готовность</button><button id="generate-page-button" class="primary-button" type="button">Запустить генерацию</button></div></div><div id="diagnose-result" class="diagnostics-grid generation-summary"><div class="diagnostic-card"><span>Группы</span><strong>${data.groups.length}</strong><small>учебных групп</small></div><div class="diagnostic-card"><span>Преподаватели</span><strong>${data.teachers.length}</strong><small>в системе</small></div><div class="diagnostic-card"><span>Аудитории</span><strong>${data.classrooms.length}</strong><small>помещений</small></div><div class="diagnostic-card"><span>Предметы</span><strong>${data.subjects.length}</strong><small>дисциплин</small></div><div class="diagnostic-card"><span>Временные слоты</span><strong>${data.time_slots.length}</strong><small>доступно</small></div></div><div class="generation-progress" hidden><i></i></div><div id="generation-status" class="status">Перед запуском проверьте входные данные.</div></section>`;
